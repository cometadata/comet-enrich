...
//! the 16-hex xxh3-64 the prototypes use (so persisted hashes stay compatible),
//! and 128-bit (32-hex) is the collision-free escape hatch. The unique set is
...
//!
//! This module is standalone: the extract stage that fills the store, the
//! `EnrichmentMethod::inputs` seam that feeds it, and the `--hash-bits` flag /
//! manifest recording / resume pinning that select the width are wired in a later
//! stage.
...
/// matches the prototypes' persisted hashes; 128-bit is the escape hatch when the
/// loud collision check trips.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum HashBits {
    /// xxh3-64, 16 hex chars. Default; matches the prototypes.
    #[default]
    Bits64,
...
///
/// At [`HashBits::Bits64`] this matches the prototypes' `hash_funder_name` /
/// `hash_affiliation`, so persisted hashes are compatible. The input is hashed
...
fn check_collision<'a>(
    ...
    if let Some(prev) = seen.insert(raw, value) {
        // Width-neutral remediation: the CLI layer owns the `--hash-bits` affordance.
        bail!(
...
impl DedupStore {
    ...
    pub fn write_jsonl(&self, path: &Path, bits: HashBits) -> Result<()> {
        // Non-atomic: `File::create` truncates up front, so a mid-loop failure (a
        ...
        // ample memory, but worth revisiting for a very large unique set.
...
mod tests {
    ...
    fn hash_64_matches_prototype_ground_truth() {
        // A real (funder_name -> funder_name_hash) row from the funders prototype
        // ground truth proves byte-for-byte hash compatibility at 64-bit.
        assert_eq!(
            hash_input(
